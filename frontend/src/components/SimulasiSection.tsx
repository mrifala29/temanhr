...
const InputPreview = () => (
  <div className="grid md:grid-cols-2 gap-4">
    {[
      ...
      <div key={i} className={(f as any).full ? 'md:col-span-2' : ''}>
...
/* ── OUTPUT (tabel) ── */
const candidates = [
  { rank: 1, name: 'Budi Santoso',  edu: 'S1 Teknik Informatika — UI',  exp: '5 tahun', email: 'budi@email.com',  wa: '0812-xxxx-1001', score: 92, status: 'Priority' },
  { rank: 2, name: 'Sari Dewi',     edu: 'S1 Sistem Informasi — ITS',   exp: '4 tahun', email: 'sari@email.com',  wa: '0813-xxxx-2002', score: 85, status: 'Priority' },
  { rank: 3, name: 'Raka Pratama',  edu: 'S1 Ilmu Komputer — UGM',      exp: '3 tahun', email: 'raka@email.com',  wa: '0821-xxxx-3003', score: 74, status: 'Good Chance' },
  { rank: 4, name: 'Anisa Putri',   edu: 'S1 Teknik Elektro — ITB',     exp: '2 tahun', email: 'anisa@email.com', wa: '0857-xxxx-4004', score: 66, status: 'Good Chance' },
];
      ...
      <table className="w-full text-xs">
        <thead>
          <tr className="bg-gray-50 border-b border-gray-100">
            ...
            <th className="text-left px-4 py-3 font-semibold text-gray-500">Nama</th>
            <th className="text-left px-4 py-3 font-semibold text-gray-500 hidden md:table-cell">Pendidikan</th>
            <th className="text-left px-4 py-3 font-semibold text-gray-500 hidden md:table-cell">Pengalaman</th>
            <th className="text-left px-4 py-3 font-semibold text-gray-500 hidden lg:table-cell">Kontak</th>
            ...
            <th className="text-center px-4 py-3 font-semibold text-gray-500">Aksi</th>
          </tr>
        </thead>
        <tbody>
          {candidates.map((c, i) => (
            ...
            >
              <td className="px-4 py-3 text-gray-400 font-medium">{c.rank}</td>
              <td className="px-4 py-3 font-semibold text-gray-900">{c.name}</td>
              <td className="px-4 py-3 text-gray-500 hidden md:table-cell">{c.edu}</td>
              <td className="px-4 py-3 text-gray-500 hidden md:table-cell">{c.exp}</td>
              <td className="px-4 py-3 hidden lg:table-cell">
                <div className="text-gray-500">{c.email}</div>
                <div className="text-gray-400">{c.wa}</div>
              </td>
              <td className="px-4 py-3 text-center">
                <span className={`inline-flex items-center justify-center w-9 h-9 rounded-full font-bold text-white text-xs ${c.score >= 80 ? 'bg-emerald-500' : 'bg-amber-400'}`}>
                  {c.score}
                </span>
              </td>
              <td className="px-4 py-3 text-center">
                <span className={`text-[10px] font-semibold px-2 py-1 rounded-full ${c.status === 'Priority' ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'}`}>
                  {c.status === 'Priority' ? 'Priority' : 'Good Chance'}
                </span>
              </td>
              <td className="px-4 py-3 text-center">
                <span className="text-xs bg-white border border-gray-200 text-gray-600 px-3 py-1.5 rounded-full cursor-default hover:border-indigo-300 transition-colors">
                  Detail
                </span>
              </td>
            </motion.tr>
  ...
  <div className="space-y-5">
    <div className="flex items-center justify-between bg-amber-50 border border-amber-100 rounded-2xl px-5 py-4">
      ...
      <span className="text-3xl font-extrabold text-emerald-600">
        92<span className="text-sm font-normal text-gray-400">/100</span>
      </span>
    </div>
